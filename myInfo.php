...
<body onLoad="showMessage( '<?php echo $_POST['message'];?>' );">
    ...
    <nav class="menubar">
        <ul class="menu">
            <li><a href="MapMain.php">
                    <h1 class="logo">Epula</h1>
                </a></li>
            <li style="margin-left: 680px;">
                <h1 class="logo">메인 화면</a>
            </li>
        </ul>
        <ul class="mydata">
	...
	if ($_SESSION['user_id']== NULL) {
		...
		echo "<h1 style='position: relative; font-size: 45px; right: 60px; bottom: 37px;'>".$_SESSION['name']."</h1>";
		echo "<h1 style='font-size: 40px;position:relative; right:140px;'>식객지수</h1>";
		echo "<h1 style='font-size: 30px; position: relative; top: 20px; left:150px;'>".$_SESSION['gourmet_score']."점</h1>";
		echo "<progress value='".$_SESSION['gourmet_score']."' max='100' style='width:400px; height:50px;'></progress>";
		echo "</div>";
		/* 이외 정보 표시 */
		// 선호 카테고리 시각화
		$favorite_food = "없음";
